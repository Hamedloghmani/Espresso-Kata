...
(the number may be zero), after the last word (the number may be zero), and between words
(at least one between any pair of neighbouring words), and then the boy glues together all the words, including "WUB",
...
def song_decoder(string: str):
    return ' '.join(filter(None, string.split('WUB')))
